...
--
-- Source of truth for BOTH deployment targets: the embedded Postgres bundled
-- with the desktop app, and hosted/on-prem multi-tenant Postgres. There is no
-- parallel SQLite schema, so a user upgrading from local to hosted carries
-- their data across without a migration rewrite.
--
