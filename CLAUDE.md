...
- `deps.py` — `WorkspaceId` (from the `X-Workspace-Id` header) and `not_implemented()` for phase-2 stubs (raises 501, deliberately distinguishable from a working endpoint with no data). Auth: `CurrentUser` resolves the session cookie via `db.auth()` (401 if missing/expired/revoked); `Member`/`Bookkeeper`/`Owner` additionally require a `workspace_members` row for the request's workspace, ranked `viewer < bookkeeper < owner` (403 `not_a_member`/`insufficient_role` otherwise). Those three are wired to the *header* `workspace_id` — `routers/workspaces.py`'s two routes identify their workspace from the URL path instead, so they call the underlying `load_membership`/`check_role` helpers directly rather than reusing `Member`/`Owner`, to avoid checking membership against a different workspace than the handler acts on. See `packages/api/tests/test_auth.py` and `db/verify_auth.sql`.
...
Implemented routers: `health`, `auth`, `accounts`, `journal-entries`, `workspaces`. Still-stub routers (501): `ai_config`, `documents`, `imports`, `categorization`, `anomalies`, `query` — this is the AI/OCR/data-entry layer, phase 2's unfinished half.
...
### Three threat models, three roles, one schema
`db/roles.sql` creates `accounting_app` (the API's role — DML only, no DDL, not the owner, so RLS actually binds), `nl_query` (generated text-to-SQL's role — read-only, statement-timed-out, reaches only the `ledger_query` schema, no privileges on `public` at all, not even `USAGE`), and `accounting_auth` (identity operations' role — `Database.auth()`, reaches only `users`/`user_credentials`/`sessions`/`workspace_members`, no reach into any ledger table). Never widen `accounting_app`'s privileges to "make something work" — if a write is structurally impossible for it (like creating a workspace, or reading across tenants to resolve a login), that's answered by a separate narrowly-scoped connection (`Database.provision()`, `Database.auth()`), not by weakening RLS.
